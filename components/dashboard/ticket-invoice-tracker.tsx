...
import { createTicket, getCompanyUsers, getEmployeesForTicket, getInvoicePermissions, getTickets, isXomCompanyActive, setInvoicePermission, updateTicket } from "@/app/actions/manage-trackers"

type Ticket = { id: string; ticketNo: string; subject: string; description?: string; priority: string; status: string; category: string; dueDate?: string; assigneeId?: string; assigneeName?: string }
...
export function TicketInvoiceTracker() {
  ...
  const [form, setForm] = useState({ subject: "", description: "", priority: "Medium", category: "General", dueDate: "", assigneeId: "" })
  ...
  const load = async () => {
    ...
    const [nextTickets, nextUsers, nextEmployees] = await Promise.all([getTickets(activeCompanyId), admin ? getCompanyUsers(activeCompanyId) : Promise.resolve([]), admin ? getEmployeesForTicket(activeCompanyId) : Promise.resolve([])])
